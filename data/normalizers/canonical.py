...
Schema TOFU canónico (una fila por dia por plataforma):
  ...
  channel_breakdown  str        JSON dict: {canal: impressions}
  device_breakdown   str        JSON dict: {dispositivo: impressions}
"""
...
def normalize_google_ads(raw_data: dict[str, Any]) -> pd.DataFrame:
    ...
    daily: dict[str, dict] = defaultdict(lambda: {
        ...
        "clicks": 0,
        "spend_micros": 0,
        "channel_counts": defaultdict(int),     # {canal: impresiones}
        "device_counts": defaultdict(int),       # {dispositivo: impresiones}
    })

    for row in raw_metrics:
        date = row["date"]
        daily[date]["impressions"] += row.get("impressions", 0)
        daily[date]["clicks"] += row.get("clicks", 0)
        ...
        daily[date]["channel_counts"][channel_label] += row.get("impressions", 0)

        device_raw = row.get("device", "UNKNOWN")
        device_label = _DEVICE_MAP.get(device_raw, device_raw)
        daily[date]["device_counts"][device_label] += row.get("impressions", 0)
    ...
    for date in sorted(daily.keys()):
        ...
        channel_breakdown_json = json.dumps(
            dict(d["channel_counts"]), ensure_ascii=False
        )
        device_breakdown_json = json.dumps(
            dict(d["device_counts"]), ensure_ascii=False
        )
